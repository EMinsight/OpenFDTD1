/*
OpenFDTD Version 3.1.1 (CUDA + MPI)
*/
...
int main(int argc, char *argv[])
{
	...
	if (io) {
		...
		if (!ierr) {
			ierr = input(fp_in, ((geom != 1) && (runMode < 2)));
			fclose(fp_in);
		}
	}
...
{
	const char usage[] = "Usage : mpiexec -n <process> ofd_cuda_mpi [-solver|-post] [-gpu|-cpu] [-hdm|-um] [-ev] [-geom|-nogeom] [-prompt] <datafile>";
