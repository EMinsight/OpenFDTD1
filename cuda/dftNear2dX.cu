...
DFT of near2d field in X plane
*/
...
void dftNear2dX(int i, int64_t adr1, int64_t adr2,
	d_complex_t *cex, d_complex_t *cey, d_complex_t *cez,
	d_complex_t *chx, d_complex_t *chy, d_complex_t *chz)
{
	if (GPU) {
		...
		dim3 grid(
			...
			CEIL(Ny + 1, near2dBlock.y));
		dftNear2dX_gpu<<<grid, near2dBlock>>>(
			commSize, commRank,
			Nx, Ny, Nz, iMin, iMax,
			i,
			Ex, Ey, Ez,
